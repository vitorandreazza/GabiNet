...
app.controller("loginCtrl", function ($scope, loginAPI, cidadaoAPI, $location, $rootScope, $cookies, $route) {
    ...
    $scope.listaAssessor = function () {
        loginAPI.getUsuarios($cookies.get('idPai'))
            .success(function (data) {
                $scope.usuarios = data;
            });
    };
    
    $scope.novoAssessor = function (usuario) {
        if (usuario.senha === usuario.confirmaSenha)
        {
            delete usuario.confirmaSenha;
            usuario.idPai = {}; 
            usuario.idPai.id = $cookies.get('idPai');
            loginAPI.postUsuario(usuario, usuario.senha)
            .success(function () {
    ...
    $scope.excluiAssessor = function (id) {
        if (confirm("Confirma a exclusão do assessor?")) {
            loginAPI.deletaAssessor(id)
            .success(function () {
                $route.reload();
            })
            .error(function () {
                alert("Erro ao excluir assessor!");
            });
        }  
    };
    
    $scope.alterarAssessor = function (usuario) {
        loginAPI.alterarAssessor(usuario)
            .success(function () {
                $route.reload();
            });
    };
    
    $scope.gerarPdf = function (mes) {
        var doc = new jsPDF();
        var x = 20, y = 0;
        cidadaoAPI.getAniversariantes(mes, $cookies.get('idPai'))
...
//                doc.text(x+85, y+35, 'PDT');
                ...
                { 
                    var dataATV = new Date(dados[i].nascimento);
                    var hoje = new Date();
                    var dia;
                    if(dataATV.getDate() >=31)
                        dia =1;
                    else
                        dia = dataATV.getDate()+1;
                    var dataString = dia + '/' + mes + '/' + hoje.getFullYear();
                    dados[i].nascimento = dataString;
                    doc.setFontSize(11);
                    doc.setFontType("bold");
                    doc.text(x, y, 'NOME:');
